...
const wrapperRerender = (node) => {
  act(() => {
    rerenderFn(node)
  })

  currentProps = node.props || currentProps
}

const fireScroll = (node) => {
  act(() => {
    fireEvent.scroll(node)
  })
}

const getTriggerEl = () => container.firstChild.childNodes[0]
const getStickyEl = () => container.firstChild.childNodes[1]
...
describe('Sticky', () => {
  common.isConformant(Sticky, { requiredProps: { active: false } })
  ...
  beforeEach(() => {
    sandbox.stub(globalThis, 'requestAnimationFrame').callsArg(0)
    sandbox.stub(globalThis, 'cancelAnimationFrame')
    container = undefined
  ...
  describe('active', () => {
    ...
    it('fires event when changes to true', () => {
      ...
      wrapperRerender(<Sticky active context={mockContextEl()} onTop={onTop} />)
      onTop.should.have.been.calledOnce()
    })
    ...
    it('omits event and removes styles when changes to false', () => {
      ...
      _.forEach(['ui', 'sticky', 'fixed', 'top'], (className) =>
        expect(getStickyEl().classList.contains(className)).to.be.true(),
      )
      ...
      scrollToTop()
      expect(getStickyEl().classList.contains('fixed')).to.be.false()
  ...
  describe('behaviour', () => {
    it('should stick to top of screen', () => {
      ...
      expect(getStickyEl().style.top).to.equal('12px')
    })
    ...
    it('should stick to bottom of context', () => {
      ...
      expect(getStickyEl().style.bottom).to.equal('0px')
    })
    ...
    it('should preserve sticky element height', () => {
      ...
      expect(getTriggerEl().style.height).to.equal('100px')
    })
  })
  describe('onBottom', () => {
  ...
  describe('pushing', () => {
    it('should push component back', () => {
      ...
      wrapperRerender(<Sticky {...positions} context={mockContextEl({ bottom: 0 })} pushing />)
      fireScroll(window)
      ...
      expect(getStickyEl().style.bottom).to.equal('30px')

      _.forEach(['ui', 'sticky', 'fixed', 'bottom'], (className) =>
        expect(getStickyEl().classList.contains(className)).to.be.true(),
      )

      expect(getStickyEl().style.top).to.equal('')
    })
    ...
    it('should stop pushing when reaching top', () => {
      ...
      expect(getStickyEl().style.top).to.equal('10px')
    })
  })
  ...
  describe('scrollContext', () => {
    ...
    it('should set a scroll context via React refs', () => {
      ...
      fireScroll(scrollContextRef.current)
      onStick.should.have.been.called()
    })
    ...
    it('should call onStick when scrollContext changes', () => {
      ...
      wrapperRerender(<Sticky scrollContext={div} onStick={onStick} />)
      mockTriggerEl({ top: -1 })
  ...
  describe('styleElement', () => {
    it('is passed to macthing element', () => {
      ...
      expect(getStickyEl().style.zIndex).to.equal('10')
    })
  })
})
