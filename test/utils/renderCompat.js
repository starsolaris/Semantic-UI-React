 ...
 */
function createWrapper(container, originalNode = null, rtlResult = null) {
  ...
  return {
    ...
    simulate(event, ...args) {
      ...
      let dispatchResult
      act(() => {
        dispatchResult = element.dispatchEvent(nativeEvent)
      })
    ...
    // Unmount
    unmount() {
      if (rtlResult && typeof rtlResult.unmount === 'function') {
        rtlResult.unmount()
      }
    },
    ...
    exactly(n) {
      return {
        descendants: (selector) => {
          ...
          if (results.length !== n) {
            throw new Error(
              `Expected exactly ${n} descendants matching "${selector}" but found ${results.length}`,
            )
          }
          return results
        },
...
function createWrapperFromElement(el) {
  ...
  return {
    ...
    simulate(event, payload) {
      ...
      let dispatchResult
      act(() => {
        dispatchResult = el.dispatchEvent(nativeEvent)
      })
...
export function shallow(node, options = {}) {
  ...
  const rtlResult = render(node, { container: customContainer, ...rest })
  return createWrapper(rtlResult.container, null, rtlResult)
}
  ...
  get() {
    ...
    return {
      have: {
        tagName(expected) {
          const actual =
            self.tagName || (self.node && self.node.tagName && self.node.tagName.toLowerCase())
          if (actual !== expected) {
        ...
        value(expected) {
          const actual =
            typeof self.value === 'function'
              ? self.value()
              : self.value || (self.node && self.node.value)
          if (actual !== expected) {
